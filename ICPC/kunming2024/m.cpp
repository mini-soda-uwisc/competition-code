...
void solve(){
    ...
    int pl = 0, pr = 0;
    ld area = 0, tot = 0, qwq = 0;
    Point<ld> pt;
    Point<ld> cir = {x, y};

    for(int i = 2; i < n; i++){
        tot += fabs(pt.cross(p[i - 1], p[i], p[0]));
    }

    while(pr < 2 * n){
        if(pr - pl >= 2){
            area += fabs(pt.cross(p[pr], p[pr - 1], p[pl]));
        }
        while(pr - pl >= 2 && pt.line_dist(p[pr], p[pl], cir) <= r){
            area -= fabs(pt.cross(p[pl], p[pl + 1], p[pr]));
            pl++;
        }

        if(pr - pl >= 2){
            if(pt.cross(p[pr], p[pl], cir) >= 0) {
                qwq = chmax(qwq, tot - area);
            }
            else{
                qwq = chmax(qwq, area);
            }
